package com.underwaterimageprocessor.imageprocessor.service.impl;

import com.underwaterimageprocessor.imageprocessor.model.OrgImage;
import com.underwaterimageprocessor.imageprocessor.repository.ImageRepository;
...
public class ImageServiceImpl implements ImageService{
    ...
    public OrgImage uploadImage(MultipartFile file, String author, String fileType) throws IOException{
        ...
        String dir = env.getProperty("upload.image.dir");

        //orgImage.setImage(byteImg);
        //orgImage.setAuthorId(author);
        orgImage.setUploadDate(ZonedDateTime.now()
                                    .format(DateTimeFormatter.ofPattern("MM/dd/yyyy hh:mm:ss")));
        orgImage.setFileType(fileType);

        orgImage = imageRepository.save(orgImage);
        String uri = ImageUtils.saveOrgImageLocal(byteImg, orgImage.getId().toString(), dir);
        orgImage.setByteURI(uri);
        imageRepository.save(orgImage);

        /*
        System.out.println(orgImage.getId());
        System.out.println(orgImage.getId().getClass());

        System.out.println(orgImage.getId());
        Optional<OrgImage> orgImageToUpdate = imageRepository.findById(orgImage.getId());
        System.out.println("2");
        orgImageToUpdate.ifPresent( s -> orgImageToUpdate.get().setByteURI(uri));
        System.out.println("3");
        orgImage = imageRepository.save(orgImageToUpdate.get());
